...
with tabs[0]:
    ...
    with col_left:
        ...
        if st.session_state.pos_cart:
            ...
            st.dataframe(cart_df[['display_name', 'qty', 'unit_price', 'total_price']], use_container_width=True)
                    ...
                    for item in st.session_state.pos_cart:
                        ...
                        if item['category'] == 'Pharmacy':
                            ...
                            c.execute("INSERT INTO sales_history (prescription_no, bill_type, doctor_name, bill_details, total_amount, doc_fee, lab_cost, center_profit, cost_price, date, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                                      (ref_no, 'Pharmacy', '-', f"{item['id']}|{item['display_name']} x {item['qty']}", item['total_price'], 0.0, 0.0, item['total_price'] - cost_tot, cost_tot, datetime.now(), 'COMPLETED'))
                            ...
                            c.execute("INSERT INTO sales_history (prescription_no, bill_type, doctor_name, bill_details, total_amount, doc_fee, lab_cost, center_profit, date, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                                      (ref_no, 'Laboratory', '-', item['display_name'], item['total_price'], 0.0, item['lab_cost'], item['center_comm'], datetime.now(), 'COMPLETED'))
                        
                        elif item['category'] in ['OPD', 'Channeling', 'Scanning']:
                            c.execute("INSERT INTO sales_history (prescription_no, bill_type, doctor_name, bill_details, total_amount, doc_fee, lab_cost, center_profit, date, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                                      (ref_no, item['category'], item.get('doctor_name', '-'), item['display_name'], item['total_price'], item.get('doc_fee', 0.0), 0.0, item.get('center_profit', 0.0), datetime.now(), 'COMPLETED'))
...
if "📦 Master Settings & Inventory" in available_tabs:
    ...
    with tabs[tab_index]:
        ...
        with st_tab1:
            ...
            st.dataframe(inv_full_df, use_container_width=True)
        ...
        with st_tab2:
            ...
            st.dataframe(labs_all_df, use_container_width=True)
        ...
        with st_tab3:
            ...
            st.dataframe(docs_all_df, use_container_width=True)
        ...
        with st_tab4:
            ...
            st.dataframe(opd_proc_all, use_container_width=True)
        ...
        with st_tab5:
            ...
            st.dataframe(all_presets_df[['id', 'preset_name']], use_container_width=True)
...
# ---------------------------------------------------------
# TAB 3: REPORTS & ACCOUNTS (WITH TOTALS ADDED AT THE BOTTOM)
# ---------------------------------------------------------
...
with tabs[rep_tab_index]:
    ...
    with main_rep_tab1:
        ...
        if not sales_full_df.empty:
            ...
            # 2. SECTION-WISE BREAKDOWN WITH TOTALS AT THE BOTTOM
            with r_tab_sections:
                ...
                with sec_lab:
                    ...
                    st.dataframe(lab_sales[['date', 'prescription_no', 'bill_details', 'total_amount', 'lab_cost', 'center_profit']], use_container_width=True)
                    st.markdown("---")
                ...
                with sec_pharma:
                    ...
                    st.dataframe(pharma_sales[['date', 'prescription_no', 'bill_details', 'total_amount', 'cost_price', 'center_profit']], use_container_width=True)
                    st.markdown("---")
                ...
                with sec_opd:
                    ...
                    st.dataframe(opd_sales[['date', 'prescription_no', 'bill_details', 'doc_fee', 'center_profit', 'total_amount']], use_container_width=True)
                    st.markdown("---")
                ...
                with sec_chan:
                    ...
                    st.dataframe(chan_sales[['date', 'prescription_no', 'doctor_name', 'doc_fee', 'center_profit', 'total_amount']], use_container_width=True)
                    st.markdown("---")
                ...
                with sec_scan:
                    ...
                    st.dataframe(scan_sales[['date', 'prescription_no', 'doctor_name', 'doc_fee', 'center_profit', 'total_amount']], use_container_width=True)
                    st.markdown("---")
            ...
            with r_tab_all:
                ...
                if not filtered_df.empty:
                    st.dataframe(filtered_df[['id', 'date', 'prescription_no', 'bill_type', 'bill_details', 'total_amount', 'status']], use_container_width=True)
    ...
    with main_rep_tab2:
        ...
        if not inv_rep_df.empty:
            ...
            with col_exp1:
                ...
                st.dataframe(reorder_df[['supplier_name', 'item_name', 'brand_name', 'stock_qty', 'reorder_level']], use_container_width=True)
                    
            with col_exp2:
                st.markdown("#### **⏰ Expired or Expiring Soon**")
                expiring_df = inv_rep_df[inv_rep_df['expiry_dt'] <= three_months]
                st.dataframe(expiring_df[['item_name', 'brand_name', 'batch_no', 'stock_qty', 'expiry_date']], use_container_width=True)
